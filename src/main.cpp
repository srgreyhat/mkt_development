    ...
    if (!fKnown) {
        ...
        if (nNewChunks > nOldChunks) {
            ...
            if (CheckDiskSpace(nNewChunks * BLOCKFILE_CHUNK_SIZE - pos.nPos)) {
                ...
                if (file) {
                    LogPrintf("Pre-allocating up to position 0x%x in mkt%05u.dat\n", nNewChunks * BLOCKFILE_CHUNK_SIZE, pos.nFile);
...
bool CheckBlock(const CBlock& block, CValidationState& state, const Consensus::Params& consensusParams, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig)
{
    ...
    if (block.vtx.empty() || block.vtx.size() > MAX_BLOCK_SIZE || ::GetSerializeSize(block, SER_NETWORK, PROTOCOL_VERSION) > MAX_BLOCK_SIZE)
        return state.DoS(100, false, REJECT_INVALID, "bad-mkt-length", false, "size limits failed");
    ...
    if (nSigOps > MAX_BLOCK_SIGOPS)
        return state.DoS(100, false, REJECT_INVALID, "bad-mkt-sigops", false, "out-of-bounds SigOpCount");
...
static bool AcceptBlockHeader(const CBlockHeader& block, CValidationState& state, const CChainParams& chainparams, CBlockIndex** ppindex=NULL, bool fProofOfStake=true)
{
    ...
    if (hash != chainparams.GetConsensus().hashGenesisBlock) {
        ...
        if (mi == mapBlockIndex.end())
            return state.DoS(10, error("%s: prev block not found", __func__), 0, "bad-prevmkt");
        pindexPrev = (*mi).second;
        if (pindexPrev->nStatus & BLOCK_FAILED_MASK)
            return state.DoS(100, error("%s: prev block invalid", __func__), REJECT_INVALID, "bad-prevmkt");
...
void UnlinkPrunedFiles(std::set<int>& setFilesToPrune)
{
    for (set<int>::iterator it = setFilesToPrune.begin(); it != setFilesToPrune.end(); ++it) {
        ...
        boost::filesystem::remove(GetBlockPosFilename(pos, "mkt"));
        boost::filesystem::remove(GetBlockPosFilename(pos, "rev"));
        LogPrintf("Prune: %s deleted mkt/rev (%05u)\n", __func__, *it);
    }
}
...
void FindFilesToPrune(std::set<int>& setFilesToPrune, uint64_t nPruneAfterHeight)
{
    ...
    LogPrint("prune", "Prune: target=%dMiB actual=%dMiB diff=%dMiB max_prune_height=%d removed %d mkt/rev pairs\n",
...
FILE* OpenBlockFile(const CDiskBlockPos &pos, bool fReadOnly) {
    return OpenDiskFile(pos, "mkt", fReadOnly);
}
...
bool static LoadBlockIndexDB()
{
    ...
    // Check presence of mkt files
    LogPrintf("Checking all mkt files are present...\n");
    set<int> setmktDataFiles;
    BOOST_FOREACH(const PAIRTYPE(uint256, CBlockIndex*)& item, mapBlockIndex)
    {
        CBlockIndex* pindex = item.second;
        if (pindex->nStatus & BLOCK_HAVE_DATA) {
            setmktDataFiles.insert(pindex->nFile);
        }
    }
    for (std::set<int>::iterator it = setmktDataFiles.begin(); it != setmktDataFiles.end(); it++)
    {
        CDiskBlockPos pos(*it, 0);
...
bool LoadExternalBlockFile(const CChainParams& chainparams, FILE* fileIn, CDiskBlockPos *dbp)
{
    ...
    try {
        ...
        CBufferedFile mktdat(fileIn, 2*MAX_BLOCK_SIZE, MAX_BLOCK_SIZE+8, SER_DISK, CLIENT_VERSION);
        uint64_t nRewind = mktdat.GetPos();
        while (!mktdat.eof()) {
            boost::this_thread::interruption_point();

            mktdat.SetPos(nRewind);
            nRewind++; // start one byte further next time, in case of failure
            mktdat.SetLimit(); // remove former limit
            unsigned int nSize = 0;
            try {
                // locate a header
                unsigned char buf[MESSAGE_START_SIZE];
                mktdat.FindByte(chainparams.MessageStart()[0]);
                nRewind = mktdat.GetPos()+1;
                mktdat >> FLATDATA(buf);
            ...
            try {
                // read block
                uint64_t nBlockPos = mktdat.GetPos();
                if (dbp)
                    dbp->nPos = nBlockPos;
                mktdat.SetLimit(nBlockPos + nSize);
                mktdat.SetPos(nBlockPos);
                CBlock block;
                mktdat >> block;
                nRewind = mktdat.GetPos();
