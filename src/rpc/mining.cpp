...
UniValue getblocktemplate(const UniValue& params, bool fHelp)
{
    ...
    if (params.size() > 0)
    {
        ...
        if (strMode == "proposal")
        {
            ...
            CBlock block;
            if (!DecodeHexmkt(block, dataval.get_str()))
            ...
            if (block.hashPrevBlock != pindexPrev->GetBlockHash())
                return "inconclusive-not-best-prevmkt";
            CValidationState state;
...
UniValue submitblock(const UniValue& params, bool fHelp)
{
    ...
    CBlock block;
    if (!DecodeHexmkt(block, params[0].get_str()))
        throw JSONRPCError(RPC_DESERIALIZATION_ERROR, "Block decode failed");
...
UniValue checkkernel(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        ...
        if (vNodes.empty())
            throw JSONRPCError(-9, "Marketcoin is not connected!");

        if (IsInitialBlockDownload())
            throw JSONRPCError(-10, "Marketcoin is downloading blocks...");
